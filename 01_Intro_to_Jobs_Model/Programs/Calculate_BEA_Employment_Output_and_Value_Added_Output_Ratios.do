...
*
*  NOTES: 			Called by Run_ARRA_Model.do - cleans data from BEA on 
*					employment, output, and value-added by industry and calculates
*					ratios needed for implementation of I-O model
/***********************************************************************************/
...
foreach year of numlist 2009(1)2019 {
	...
	* Clean output data
	...
	import excel using ${workdir}/Data/_BEA_Gross_Output_by_Industry/BEA_Gross_Output_by_Industry_Cleaned_`year'.xls, clear
	drop if _n < 7
	...
	*drop D E
	...
	save ${workdir}/Work/BEA_Gross_Output_by_Industry_Cleaned_`year', replace

	* Clean employment data

	import excel using ${workdir}/Data/_BEA_Employment_by_Industry/BEA_Employment_by_Industry_Cleaned_`year'.xls, clear
	...
	save ${workdir}/Work/BEA_Employment_by_Industry_Cleaned_`year', replace
	
	
	* Clean value added data

	import excel using ${workdir}/Data/_BEA_Value_Added_by_Industry/BEA_Value_Added_by_Industry_Cleaned_`year'.xls, clear
	...
	save ${workdir}/Work/BEA_Value_Added_by_Industry_Cleaned_`year', replace
	
	
	* Merge datasets
	
	use ${workdir}/Work/BEA_Gross_Output_by_Industry_Cleaned_`year', clear
	
	merge 1:1 indnum using ${workdir}/Work/BEA_Employment_by_Industry_Cleaned_`year'
	sort indnum
	drop _merge

	merge 1:1 iocode using ${workdir}/Work/BEA_Value_Added_by_Industry_Cleaned_`year'
	drop if _merge != 3
	drop _merge
	...
	*replace employment = 0 if iocode == "ORE"
	...
	gen emp_output_ratio`year' = employment / output if realestate == 0
	replace emp_output_ratio`year' = employment2 / output2 if realestate == 1

	gen va_output_ratio`year' = valueadded / output

	sort indnum
	
	drop realestate employment2 output2
	
	rename output output`year'
	rename employment employment`year'
	rename valueadded valueadded`year'

	save ${workdir}/Work/BEA_EO_and_VAO_Ratios_by_Industry_Cleaned_`year', replace
